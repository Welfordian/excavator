        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST">
                            <input class="form-control" type="text" name="name" placeholder="Middleware Name" value="{{ $middleware->name }}"/>
                            <select id="listed-routes" multiple="multiple">
                                @foreach ($routes as $route)
                                    <option value="{{ $route->id }}" @if(in_array($route->id, $middleware->routes)) selected @endif>{{ $route->uri }}</option>
                                @endforeach
                            </select>
                            ...
                            <div id="editor">{{ $middleware->logic }}</div>
